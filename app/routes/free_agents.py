...
if default_owner:
    ...
    st.caption(
        ...
        "each other in `DP`. Eligibility is ESPN's own `eligiblePositions`. "
        "**Replacing a starter needs more than a shared slot**: dropping him vacates "
        "the slot he is standing in, so the newcomer has to be able to fill that one "
        "too — which is why a quarterback is never offered as a swap for your "
        "starting running back, however far he out-projects him. Measured "
