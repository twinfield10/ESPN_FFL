...
from Scripts.filter_utils import get_any_records, exclude_most_recent_week
...
def get_opp_lineup(
    ...
    lineup = []
    for box_score in box_scores:
        if team == box_score.away_team:
            lineup = box_score.home_lineup
        elif team == box_score.home_team:
            lineup = box_score.away_lineup
    return lineup
...
def get_top_proj_players(lineup: List[Player], slot: str, n: int) -> List[Player]:
    """Takes a list of players and returns a list of the top n players based on points scored."""
    # Gather players of the desired position
    eligible_players = []
    for player in lineup:
    ...
    return sorted(eligible_players, key=lambda x: x.projected_points, reverse=True)[:n]
...
def get_best_proj_lineup(league: League, lineup: List[Player]) -> float:
    """Returns the score of the best possible lineup for team during the loaded week."""
    # Save full roster
    saved_roster = copy(lineup)
    ...
    for slot in sorted(league.roster_settings["starting_roster_slots"].keys(), key=len):
        num_players = league.roster_settings["starting_roster_slots"][slot]
        best_players = get_top_proj_players(saved_roster, slot, num_players)
        best_lineup.extend(best_players)
...
def get_idiot_score(league: League, lineup: List[Player]) -> float:
    """
    Returns the lineup efficiency of a team.
    Lineup efficiency is defined as the team's actual score divided by it's best possible score.
    """
    max_score = get_best_proj_lineup(league, lineup)
    real_score = np.sum(
        [player.projected_points for player in lineup if player.slot_position not in ("BE", "IR")]
    )
    return real_score / max_score
...
def h2h_build(
    df: pd.DataFrame, owner1: str, owner2: str
) -> pd.DataFrame:
    gow_df = df.query(
        f"team_owner == '{owner1}' & opp_owner == '{owner2}' & is_meaningful_game == True"
    )
    ...
    gow_df = gow_df.groupby(['team_owner', 'opp_owner']).agg(
        games=('week', 'count'),
        wins = ('win', 'sum'),
        losses = ('lose', 'sum'),
        ties = ('tie', 'sum'),
        PF = ('team_score', 'mean'),
        PA = ('opp_score', 'mean'),
        margin = ('score_dif', 'mean')
    )

    gow_df['win_pct'] = gow_df['wins'] / gow_df['games']

    gow_df[['games', 'wins', 'losses', 'ties', 'win_pct', 'PF', 'PA', 'margin']]
...
def season_stats_analysis(
    ...
    print(
        "Most K pts this season            - {:.1f} pts - {}".format(
            *get_leader_str(
                df_current_year.groupby("team_owner")["K_pts"].mean().to_dict().items(),
                high_first=True,
            )
        )
    )
...
def all_time_stats_analysis(
    league: League, df: pd.DataFrame
) -> None:
    """Display all time-bests and -worsts.
    ...
    current_matchup_period = league.settings.week_to_matchup_period[league.current_week]
    df = df.query(f"(year < {league.year}) or (year == {league.year} and week <= {current_matchup_period - 1})")
    df_current_year = df.query("is_meaningful_game == True")
    ...
    print("----------------------------------------------------------------------------------------")
    print(
        "|          {} ({:2.0f}-{:2.0f}) All Time Analysis                                 |".format(
            league.name , min_yr, max_yr, week
        )
    )
    print("----------------------------------------------------------------------------------------")
    ...
    print(
        "Longest Win Streak             - {:.0f} gms - {}".format(
            *get_leader_str(
                df_current_year[["team_owner", "streak"]].values, high_first=True
            )
        )
    ...
    print(
        "Longest Loss Streak            - {:.0f} gms - {}".format(
            *get_leader_str(
                df_current_year[["team_owner", "streak"]].values, high_first=False
            )
        )
    ...
    print(
        "Most RB/WR/TE Pts             - {:.1f} pts - {}".format(
            *get_leader_str(
                df_current_year.groupby("team_owner")
                ["RB_WR_TE_pts"]
                .mean()
    ...
    print(
        "Fewest RB/WR/TE Pts           - {:.1f} pts - {}".format(
            *get_leader_str(
                df_current_year.groupby("team_owner")
                ["RB_WR_TE_pts"]
                .mean()
    ...
    print(
        "Fewest Bench Pts              - {:.1f} pts - {}".format(
            *get_leader_str(
                df_current_year.groupby("team_owner")
                ["bench_points"]
                .mean()
