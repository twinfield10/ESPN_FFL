...
- **There is a season model, TOMCAT, and it is measured.**
  ...
  **That last clause was true when written and is not true now.** The quarterback
  abstention was lifted on 2026-08-07, described seventy lines below, when the depth
  chart entered the veteran arm -- `season.ABSTAIN_POSITIONS` is `()`. So the passing
  interval does reach a board: every top quarterback on the 2026 boards carries
  `pts_p10`/`pts_p90`. Re-measured 2026-08-26 it is **58.9%** against a nominal 80%, and
  it misses asymmetrically -- 24.5% below p10 against 16.6% above p90 -- so it is too
...
| Issue | Location |
|---|---|
| **Two leagues score realised weeks in ways this pipeline cannot reproduce, and `actual_unpriced` now measures both.** Found on its first real run, 2026 week 1, and they are *different* defects. `john_pc_league` is **+5.00 on one player** — five long-touchdown bonuses (`PTD50`, `RTD40/50`, `RETD40/50`) that are simply unmapped, so the fix is a stat id; `john_atl_league` has a smaller version of the same thing (+1.00). `fields_league` is the interesting one: **−3.14 across all 10 played rows, every single one negative**, because it prices yardage as an *every N yards* step (1 per 25 passing, 1 per 10 rushing/receiving) and `REPL_SCORING` models it as a **linear rate**. Ours therefore exceeds ESPN's on every realised line — Sam Darnold's 13 passing yards are 0.52 to us and **0.0** to ESPN, because 13 yards buys no complete 25-yard block. That one is not a missing rule and cannot be fixed with a column name: a step function of a realised stat is a different object from a rate, and on the *projection* side the linear rate is arguably right anyway, since a projection is a continuous expectation. Which is why it was invisible for the entire life of the repo and took an actuals column to find. `LIVE_Points` is unaffected — it uses ESPN's number for a finished game. → [plan 34](plans/34-stat-first-audit.md) | `Scripts/live.py:_report_unpriced`, `scrape_player_stats.REPL_SCORING` |
| ~~**FantasyPros returned 60 players.**~~ **Fixed 2026-08-24.** Anonymously FantasyPros serves ten rows per position behind a registration fence, so the season scrape returned 60 players and `FP_` was imputed for 98% of the board — meaning "FantasyPros" *was* ESPN wearing a badge for everyone outside the top ten at each position. A **free** account lifts it: **592 rows**, D/ST from ten teams to all thirty-two, and FantasyPros real for **100% of the top 50 ADP** against 59% before. Realised weight on live rows moved from an ESPN-dominated 0.55–0.65 to a genuine ESPN 0.33 / FP 0.45 split. This **retired [plan 20](plans/20-consensus-sources.md)** on 2026-08-24. Its +0.027 was measured on non-imputed cells only, so the sample was the top ten per position — where every source agrees. Measured at 592 players, ESPN-FP disagreement is **6.0% inside ADP 50 against 31.6% outside 150**. **Re-measured the same day** at 80.1% coverage: FantasyPros' marginal value roughly doubles (+0.027 → **+0.058**; +0.109 → **+0.180** partialled) and it is no longer the least independent source — Pinnacle is. **TOMCAT's own G0 verdict strengthened**, +0.318 → **+0.371** partialled, because the old FantasyPros column was largely ESPN imputed through the mean and was counting ESPN twice against the model. ~~What remains owed is [plan 03](plans/03-projection-source-coverage.md)'s weight re-tune, frozen until after the drafts~~ — **the weight re-tune ran 2026-08-27 and was rejected; the weights do not move and there was nothing to freeze.** This entry's own finding is what made the verdict worth having: FantasyPros at 90% real instead of 13% is four times the fittable sample, and the weights are *still* not identified, so the binding constraint was never coverage. What remains owed is **G1 against the shipped season head**, which has never been run: G1's `USG` column is the deliberately crude trailing baseline in `Scripts/usage/baseline.py`, not `season.py` — and the weight re-tune inherited exactly that limitation, which is why its large `USG` result is a warning for [plan 19](plans/19-weekly-usage-model.md) rather than a weight change. The scrape was also **never wired into the nightly** and was ten days stale when found. | `scrape_FP.py` |
| **TOMCAT is not team-aware, and its own column is not reconciled.** A team cannot throw for 5,652 yards and catch 3,718, but TOMCAT projects exactly that: `sum(USG_receivingYards) / sum(USG_passingYards)` runs **0.658 (LV) to 1.704 (CLE)** across the 32 teams, with 23 of them off by more than 5%. ESPN holds the identity exactly (1.000 on every team, all three pairs); FantasyPros drifts 0.979–1.206. `reconcile_team_totals` fixes **`TRUE_` only** — deliberately, so `points_delta` against ESPN stays a real comparison — so the blend on the board is consistent and the `TOM` column beside it is not. **This contaminates any direct read of TOMCAT's disagreement with the market**: measured on the 2026 board, a player's position-adjusted TOMCAT-vs-market gap correlates with his *team's* accounting drift at **rho −0.502 for quarterbacks** (a QB whose receivers are over-projected is marked down by the arithmetic, not by the model) and +0.409 for running backs. Reconciling inside the model, rather than after the blend, is the fix; it moves `USG_` and so is post-draft work. → [plan 31](plans/31-team-coherent-tomcat.md), which locates the cause a layer deeper: a missing **team snap budget**, with two-QB teams summing to 21.0 projected quarterback-games and one-QB teams to 12.5 against a 17-game slate. | `Scripts/usage/season.py`, `season_projections.py:1156` |
...
| **The blend loses to its own inputs on rushing touchdowns.** `TRUE_rushingTouchdowns` is worse than ESPN by **+2.4%**, FantasyPros by **+5.1%** and Pinnacle by **+6.0%** MAE, paired on non-imputed cells over 5,257 player-weeks from all nine 2025 stores, and it survives all three populations. The fantasy-point MAE for the same league-season reads **-2.2%**, a clean win — yardage carries the variance, so a points number cannot see it. Cause is measured rather than guessed: rush touchdowns per carry persist year over year at **+0.260**, so four sources averaging four extrapolations of mostly-noise is worse than taking one. The blend is 9.4% *better* than BetOnline on the same stat, which points at BetOnline's `anytimeTouchdown` split. **Deliberately not fixed** — it is a source-bias question and bias comes after the stat lines are right. `python -m Scripts.lab.accuracy` | `Scripts/lab/accuracy.py`, `projection_utils.py` |
| ~~**Every efficiency shrinkage constant is below its measured floor.**~~ **Withdrawn 2026-08-27 — the inference was backwards and the walk-forward says so.** Solving `n/(n+k) = r` on an *observed* correlation gives `k >= k_opt`, with equality only if the true rate is perfectly stable, so real drift inflates it: the number is a **ceiling**, and a shipped constant below it is where a calibrated one belongs. Three experiments moved the rates up to it and the pre-committed rule rejected all three — mean within-position Spearman **-0.0018** at the ceiling, **-0.0009** for touchdown rates alone, **-0.0012** at a 2x midpoint, monotone in the shrinkage and worst at quarterback. Interception MAE improves 3.4% and does not pay for it. The persistence table stands as a *ranking* of what is forecastable. → [plan 34](plans/34-stat-first-audit.md) F3. Original note follows. | Inverting the credibility identity `n/(n+k) = r` at the median denominator over 13,288 consecutive player-season pairs, 2016-2025: catch rate implies k=54 against a shipped 40, yards per carry 155 against 60, yards per attempt **618 against 150**, interceptions per attempt **1394 against 300**. The implied number is a *floor* — it assumes a perfectly stable underlying rate, and drift only raises it — so the model shrinks efficiency too little at all eight rates, by 1.4x to 4.6x. Refitting moves `USG_` and therefore `TRUE_`. `python -m Scripts.lab.persistence` | `Scripts/usage/features.py` |
| ~~**Six scored rules in `john_pc_league` are mapped, present and identically zero.**~~ **Projection half built 2026-08-27** — `Scripts/usage/milestones.py` derives the expected band games from a fitted weekly distribution, walk-forward calibrated at 0.90-1.26 on population totals and 0.87-1.07 per per-game-mean bin. Worth +17.6 points to the top receiver in that league. The **actuals** half is still owed and is a naming problem. Building it found a bug in shipped code: past `CV^2 = s/(1-s)` the zero-inflated mixture in `predictive._reparameterise` has a negative conditional variance, was clipped, and collapsed to a point mass — **81 of 3,508 board interval cells (2.31%) ship p10 == p90 today**, including the top receiver at 1,631 yards exactly. Guarded. → [plan 34](plans/34-stat-first-audit.md). Original note follows. | The six yardage-milestone bonuses (`rushingYards100-199Game` and siblings, worth 1-5 points each) are **0.0 for all 3,095 player-weeks — in the actuals as well as the projections**, so the name this pipeline reads is not the name ESPN's breakdown uses. Plan 01 catches an *unmapped* rule; this one is mapped, so nothing reported it. It cost that league a median 0.48 points a row and was invisible because the weekly points patch absorbed it. `report_silent_zero_stats` now warns. **The projection half needs a model rather than a column name**: a bonus is a non-linear function of the stat line, so it cannot be a column times a constant, and what it wants is a per-game distribution counted across the threshold -- the same object `Scripts/dst/model.py` already integrates for `PA_TIERS`. Noted, not queued. | `scrape_player_stats.py:score_to_lab_dict` |
...
| ~~`clean_pinny()` is ~27% commented out — the pivot, TD-split and no-vig `adjust_value()` are inert.~~ **Deleted 2026-08-27** with plan 35. The scraper does that work and writes the wide frame; the commented copy carried a third instance of the juice coefficient, which is worse than no copy because a reader had to decide which one shipped. Measured, Pinnacle *does* still contribute real lines for the players it covers; the problem is coverage (plan 03), not absence. | `projection_utils.py` |
...
| ~~ESPN sometimes doubles yardage projections; worked around by halving when `ESPN > FP*1.75 and > 40`.~~ **Rewritten 2026-08-26, and the old rule could not fire.** It compared ESPN against FantasyPros *before* the FantasyPros imputation ran, so for any player FantasyPros had no line for it compared against NaN, returned False, and the doubled value reached the store — and FantasyPros served sixty players in 2025 behind a registration fence. Deebo Samuel's week 6 is in `Data/Store/2025/` at **136.3 receiving yards** against an ESPN `projPoints` consistent with 68. `halve_doubled_espn_yardage` needs no second source: ESPN publishes a stat line *and* a point total, so when they disagree the disagreement is ESPN's own and the total is the authoritative half. Fires on 3-8 rows per league-season. Still a correction for a known factor-of-two rather than a fix. → [plan 34](plans/34-stat-first-audit.md) | `projection_utils.py` |
| ~~**The juice formula is one expression with two coefficients, and neither is derived.**~~ **Fixed 2026-08-27.** There were *three* copies — 0.5 in `scrape_BOL.py`, 0.25 in `scrape_pinnacle.py` (changed mid-2025 in commit `c3b4d16` with no explanation), and a commented-out 0.5 in `clean_pinny()`. All of it now derives once in `Scripts/market.py`: proportional de-vig, `line + Phi^-1(q) * sigma(line)` for yardage, and an inversion of `P(N >= k) = q` for a count. The dispersion is fitted for all ten markets rather than the three `milestones.py` covered. **Two defects the evaluation had missed.** Pinnacle's touchdown column ran **−0.698 to 1.124** — 14 of 421 player-weeks projected a negative number of touchdowns — because `0.5 + Juice_Diff * 0.125` has no floor at a longshot price; and BetOnline's interception line is 0.5 in every archived row, so it too was a probability wearing a count's name, calibrated at 0.712. **Measured against realised 2025 outcomes:** BOL QB interceptions **0.712 → 1.011**, PINNY RB rushing TDs **0.679 → 0.996**, receiving **0.591 → 0.894**, and the four BetOnline touchdown columns move the 6.4% the de-vig is worth. The ladder's variance now ships as `proj_<stat>_sd`. → [plan 35](plans/35-market-lines-and-vig.md) | `Scripts/market.py` |
| **The season/draft path removes no vig at all.** `Scripts/scrape_pinnacle_season.py` computes `no_vig_over_prob` and `Scripts/season_projections.py:_pivot_props` then projects the raw `line`, discarding it. Same class of defect as the weekly one [plan 35](plans/35-market-lines-and-vig.md) fixed, on the draft board rather than the weekly frame — and left alone deliberately, because it was found mid-camp and the season conversion needs a *season-total* dispersion rather than the weekly one. | `scrape_pinnacle_season.py`, `season_projections.py` |
| ~~**Both scrapers overwrite their raw price file, so a price decision can never be scored.**~~ **Fixed 2026-08-27**, and the damage is permanent for seasons already scraped. `BetOnline_AllProps_Raw.parquet` and `Raw_Pinnacle_New.csv` were rewritten on every run, so the only prices the repo holds for 2025 are the last scrape — **one game, 624 rows**. Every earlier week survives only as the derived `proj_` column, which cannot be re-derived under a new formula, which is why most of plan 35 is verified on arithmetic rather than on outcomes. Both scrapers now keep a per-week copy. | `scrape_BOL.py`, `scrape_pinnacle.py` |
| ~~**BOL splits `anytimeTouchdown` 100% to rushing for QB/RB and 100% to receiving for WR/TE.**~~ **Fixed 2026-08-27.** It was measured: `BOL_receivingTouchdowns` calibrated at **0.0** on 910 RB player-weeks. Pinnacle split by yardage share instead, which needed *both* yardage columns and so gave a pure receiver no touchdown projection at all. Both books' anytime totals are now split by the ESPN/FantasyPros ratio in `projection_utils.reallocate_book_touchdowns` — the first point all four sources are on one frame. Blend calibration at RB: receiving **0.597 → 0.897**, rushing **1.099 → 1.022**; five of six positions improve. Worth **zero points** (all nine leagues score both types at 6.0, checked against `scoring.csv`) and it makes receiving-TD MAE **+2.1%** worse on purpose, because that stat's MAE is minimised by projecting zero. → [plan 34](plans/34-stat-first-audit.md) F2 | `projection_utils.py`, `Scripts/market.py` |
