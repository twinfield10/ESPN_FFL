...
| **The juice formula is one expression with two coefficients, and neither is derived.** 0.5 in `scrape_BOL.py:454`, 0.25 in `scrape_pinnacle.py:348` — the latter changed mid-2025 in commit `c3b4d16` with no explanation. It is also scaled by the line's **level** when it should scale by its **standard deviation** (CV runs 0.81 to 0.44 across the rushing range), and `sigma(line)` already exists in `Scripts/usage/milestones.py`. Evaluated 2026-08-27 and nothing built. Measured consequences: **BetOnline never de-vigs** (two-way probabilities sum to a median 1.0640) and **Pinnacle uses P(at least one TD) as an expected count** (low by 1.265x at RB), so on 2025 the two run 1.184/1.268 high and 0.657 low respectively while ESPN sits at 0.98 on the same rows. **And BetOnline posts a 2-17 threshold ladder whose variance the pipeline computes and discards.** → [plan 35](plans/35-market-lines-and-vig.md) | `scrape_pinnacle.py`, `scrape_BOL.py` |
