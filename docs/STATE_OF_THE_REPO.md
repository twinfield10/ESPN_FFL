...
| **BOL splits `anytimeTouchdown` 100% to rushing for QB/RB and 100% to receiving for WR/TE.** Crude for pass-catching backs, and measured: `BOL_receivingTouchdowns` calibrates at **0.0** on 910 RB player-weeks while rushing stays 40% high after de-vigging. Pinnacle splits by yardage share instead, which needs both yardage columns and so gives a pure receiver *no* touchdown projection at all. Deliberately left by [plan 35](plans/35-market-lines-and-vig.md), which fixed the price arithmetic and not the allocation → [plan 34](plans/34-stat-first-audit.md) F2, measured there as worth 0.597 → 0.891. | `scrape_BOL.py`, `scrape_pinnacle.py` |
