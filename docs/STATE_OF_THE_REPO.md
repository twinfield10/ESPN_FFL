...
| **Six scored rules in `john_pc_league` are mapped, present and identically zero.** The six yardage-milestone bonuses (`rushingYards100-199Game` and siblings, worth 1-5 points each) are **0.0 for all 3,095 player-weeks — in the actuals as well as the projections**, so the name this pipeline reads is not the name ESPN's breakdown uses. Plan 01 catches an *unmapped* rule; this one is mapped, so nothing reported it. It cost that league a median 0.48 points a row and was invisible because the weekly points patch absorbed it. `report_silent_zero_stats` now warns. Needs one live probe of `projected_breakdown` to fix. | `scrape_player_stats.py:score_to_lab_dict` |
