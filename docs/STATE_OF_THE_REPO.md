...
| **Six scored rules in `john_pc_league` are mapped, present and identically zero.** The six yardage-milestone bonuses (`rushingYards100-199Game` and siblings, worth 1-5 points each) are **0.0 for all 3,095 player-weeks — in the actuals as well as the projections**, so the name this pipeline reads is not the name ESPN's breakdown uses. Plan 01 catches an *unmapped* rule; this one is mapped, so nothing reported it. It cost that league a median 0.48 points a row and was invisible because the weekly points patch absorbed it. `report_silent_zero_stats` now warns. **The projection half needs a model rather than a column name**: a bonus is a non-linear function of the stat line, so it cannot be a column times a constant, and what it wants is a per-game distribution counted across the threshold -- the same object `Scripts/dst/model.py` already integrates for `PA_TIERS`. Noted, not queued. | `scrape_player_stats.py:score_to_lab_dict` |
