...
| Nightly refresh failed with `refusing to run off branch '<name>'` | Working as designed. `run_daily_refresh.sh` has no `git checkout` — it runs whatever is in the working tree, so a branch left checked out overnight would rebuild all nine boards and push them to S3 unmerged. Run `git checkout main`; the next run catches up, because every pull is a full snapshot. `ALLOW_ANY_BRANCH=1` overrides it for a deliberate run off a branch. |
