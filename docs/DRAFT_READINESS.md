...
| ~~**Both book sources are 13 days stale**~~ ([36](plans/36-sportsbook-scrapes.md)) | **Fixed 2026-08-27, and the diagnosis in this row was wrong.** Both books really were last written **2026-08-14** while everything else refreshed at 06:00, and under an equal-vote blend that is a stale *opinion* carrying a fifth of those players' projections rather than a missing column — which is why nothing flagged it. But the missing `__main__` guard was never the cause. The stale files come from `Scripts.scrape_pinnacle_season` and `Rscript R/GetSeasonProps.R`, **both of which already worked**; nobody had added them to the nightly. The two guardless scrapers write *weekly* props and are both broken, so guarding them and adding them — as this row proposed — would have failed the nightly every night before the draft. Both books are now nightly stages and were refreshed the same day | **Done.** `python -m Scripts.refresh_status` now names every source and its age, so this cannot recur silently |
