...
| # | Plan | Status | Why it mattered / what is left |
| 35 | [Market lines and vig](35-market-lines-and-vig.md) | **Complete 2026-08-27 — built the same day it was evaluated, and the build found two more defects than the evaluation did** | **Two of the five sources are sportsbooks, and a market is a line plus a price rather than a projection.** Both decisions — remove the margin, convert a threshold to an expectation — were made in two files, differently, and neither had been measured. Now made once, in `Scripts/market.py`. **BetOnline never de-vigged**: two-way implied probabilities sum to a median **1.0640**. The plan said the one-sided ladder could be "normalised to its own total"; that is a **no-op by construction**, so its hold was measured against the two-way line beside it instead and comes out at **1.0649** — the same 6.4%, so one number de-vigs both shapes. **The juice nudge was one formula with three coefficients** (0.5 in `scrape_BOL.py`, 0.25 in `scrape_pinnacle.py`, and a commented-out 0.5 in `projection_utils.py`) scaled by the line's **level** when it should scale by its **standard deviation** — Kyle Pitts' 54.5-yard receiving line moved **+6.27 yards** on a price tilt where the derivation moves it +2.41. **The largest error was a probability wearing a count's name, and it was live on a stat the evaluation did not check.** Pinnacle's touchdown column ran **−0.698 to 1.124** with 14 of 421 player-weeks projecting a *negative* number of touchdowns; BetOnline's interception line is 0.5 in every row and calibrated at **0.712**. The plan proposed a flat positional multiplier (E[N]/P(>=1) = **1.2650** at RB, measured over 174,374 player-weeks); measurement rejected it — the true ratio runs 1.0 to 1.6 across the rate range, so inverting the count distribution ships instead, calibrated to **1.02–1.05** across that range. **Measured against realised 2025 outcomes, every recoverable market moves toward 1.00**: BetOnline QB interceptions **0.712 → 1.011**, Pinnacle RB rushing TDs **0.679 → 0.996**, receiving **0.591 → 0.894**, and the four BetOnline touchdown columns move the 6.4% the de-vig is worth. **The ladder's variance is now claimed rather than discarded** — `proj_<stat>_sd`, exact for a count rooted at 1 — and it reads **1.64x** the historically fitted dispersion, which is emitted beside the projection and not substituted into it because the archive cannot separate a too-narrow fit from a book shading its long shots. The plan also asked to prefer the ladder over the line for the *mean*; that was **rejected**, because `sum(threshold * P(bucket))` on a yardage ladder runs 0.77 to 1.81 against the book's own line. Left: the touchdown **allocation** (100% of a back's anytime market goes to rushing, so BOL@RB receiving calibrates at 0.0 on 910 player-weeks) belongs to [34](34-stat-first-audit.md) F2, and the season/draft path is untouched mid-camp on purpose. **Both scrapers now archive raw prices per week** — they overwrote them, which is why only one game of 2025 prices survived and most of this could not be scored end to end |
| 34 | [Stat-first audit](34-stat-first-audit.md) | **Complete 2026-08-27 — two of the three owed items closed by rejecting the audit's own proposal, and F2's touchdown allocation landed the same day** | The premise is that a projection is a **stat line** and points are what a league's rules do to one. Mostly true, and an audit found **seven places it was not**. Two new measurements, both of which found something. **The shipping blend had never been scored per stat** -- every scored evaluation in the repo judges TOMCAT or a calibration curve -- and per stat it *loses* on `rushingTouchdowns`, by +2.4% against ESPN, +5.1% against FantasyPros and +6.0% against Pinnacle, on all three populations, while the fantasy-point MAE for the same league-season reads a clean **-2.2% win**. Points MAE cannot see it because yardage carries the variance. **And volume persists where efficiency does not**: carries/game +0.895 year over year against +0.260 for the rate it is multiplied by, and every one of the eight shipped `SHRINKAGE_K` constants sits **below** its measured credibility floor, by 1.4x to 4.6x. **The blend was discarding volume entirely** -- four sources publish attempts, completions and targets and `TRUE_` had none of them, so no coherence check was possible and plan 31's team attempt budget could not be expressed. Now blended, verified additive at max |delta| **0.0** on every `<prefix>_Points` column across all nine 2026 boards. The team budget turns out to **pass** at 59.1-65.0 plays a game over 32 teams. **The weekly path patched points with a scalar that had no stat behind it** -- ESPN's unpriced residual added to every source's total, `TRUE_Points` included -- and it was camouflage: underneath it, ESPN's doubled yardage was escaping a correction that compared it against a FantasyPros line that was usually not there yet (Deebo Samuel week 6 stored at **136.3** receiving yards against an ESPN total consistent with 68), and six scored milestone-bonus rules in one league are mapped, present and **identically zero for all 3,095 player-weeks**. Also **a QB interval excused by an abstention that no longer exists**: `passingYards` covers **58.9%** against a nominal 80% and the excuse in two places said the model abstains at quarterback, which stopped being true on 2026-08-07. **F2's touchdown allocation landed 2026-08-27**, on top of [35](35-market-lines-and-vig.md) — the audit's own largest error, closed. A book prices *any* scrimmage touchdown; BetOnline sent 100% to rushing for every back (`BOL_receivingTouchdowns` calibrated at **0.0** on 910 player-weeks) and Pinnacle split by yardage share, which needed *both* yardage columns and so gave a pure receiver nothing at all. Split by the ESPN/FantasyPros consensus — the only sources that project the two types separately, so no new join and no fitted parameter — blend calibration at RB goes **0.597 → 0.897** on receiving and **1.099 → 1.022** on rushing, reproducing this audit's own predictions of 0.891 and 1.046; five of six positions improve. Worth **zero points** (all nine leagues score both types at 6.0, now checked against `scoring.csv` rather than asserted) and receiving-TD MAE **+2.1%** worse *on purpose*, because that stat's MAE is minimised by projecting zero and BetOnline was doing exactly that. No provenance gate and no position rule needed, both measured: a `MEAN_`-imputed cell's own split already equals this ratio (max difference **0.0** over 2,609 rows), and a passer's consensus receiving share is 0. Left: source bias adjustment and the weight re-tune, deferred until the stat lines were right — **that condition is now met** |
|---|---|---|---|
...
- **[34](34-stat-first-audit.md)'s owed items.** Three, in order of how cheap they are.
  ...
  nominal 80% and misses asymmetrically.~~ **Resolved 2026-08-27: withdrawn.** The miss is
  the skew, not the width -- the outcome is left-skewed at 0.57 and a Gamma at the matching
  shape gives 1.57 -- so no refit works. The published interval is withdrawn and the points
  interval, which cannot be, is flagged. A left-skewed family or [28](28-outcome-distributions.md)'s
  room-level draw is what would fix it.
