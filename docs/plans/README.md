...
| 35 | [Market lines and vig](35-market-lines-and-vig.md) | Evaluated 2026-08-27, nothing built | **Two of the five sources are sportsbooks, and a market is a line plus a price rather than a projection.** Turning one into a number needs the margin removed and a threshold converted to an expectation; both decisions are made in two files, differently, and neither had been measured. **BetOnline never de-vigs** -- two-way implied probabilities sum to a median **1.0640** and nothing normalises them. **Pinnacle de-vigs and then uses P(at least one TD) as an expected count**, which is low by exactly the multi-TD share: E[N]/P(>=1) is **1.265** for backs, 1.295 on priceable weeks, with 22.5% of their scoring weeks being multi-TD. Measured on 2025, on the rows each book really priced: BetOnline runs **1.184** at RB and **1.268** at WR, Pinnacle **0.657** at RB, while ESPN sits at 0.98 and 0.91 on the same players. Two books 18-34% out in **opposite** directions, cancelling in the blend by accident -- and that is the cause of plan 34's touchdown mis-calibration. The juice nudge is also **one formula with two coefficients** (0.5 in `scrape_BOL.py:454`, 0.25 in `scrape_pinnacle.py:348`, already flagged as an unexplained mid-season change) scaled by the line's **level** when it should scale by its **standard deviation** -- CV runs 0.81 to 0.44 across the rushing range, so it over-adjusts big lines. The largest item is unclaimed rather than broken: **BetOnline posts a ladder of 2-17 thresholds per player-stat-week**, `value_calc` already differences it into `P(exactly k)`, and the function returns the mean and **discards the variance** -- then the coalesce prefers a single juice-nudged line over the ladder wherever both exist. Nothing else in the repo has a market-implied dispersion at player-week grain, and two plans currently fit one from history instead |
