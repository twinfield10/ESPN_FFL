...
| 18 | [Season usage model](18-season-usage-model.md) | **Shipped, and carrying real weight** | The draft head, on all nine boards. **Best-covered source in the pre-season blend** — 23.1% real against ESPN's 13.1%. **Rookie arm is the win** — ρ ≈ 0.61 against ~0. Wired at 0.0 so turning it on would be one number, then turned on the same day: `WEIGHTS` is an equal three-way split of ESPN, FantasyPros and `USG`, and the QB abstention was lifted once the depth chart entered the veteran arm. This row said "weight 0.0" until 2026-08-14 and was wrong for a week |
